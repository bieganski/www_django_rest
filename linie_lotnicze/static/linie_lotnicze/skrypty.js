    ...
    $.getJSON("/ajax/loty/", {'data': data})
        .done(function (lista_lotow) {
            ...
            for (let i = 0; i < lista_lotow.length; ++i) {
                if (lista_lotow[i].fields.zaloga != null) {
                    pilot_imie = "PRZYPISANY " + ' (kliknij, aby zmienić)';
                    //
                    // UWAGA
                    // chcialem zrobic zeby się pokazywał jaki jest przypisany,
                    // ale to wymaga innego serializera Modelu do JSONA niż ten,
                    // którego użyłem (trzeba obslugiwac ForeignKey), a z innym mialem problemy.
                    //
...
function ustaw_nav() {
    ...
    $('nav').append('<a href="#" onclick="main_piloci(); return false;" id="nav_piloci">Pokaż loty</a>');
    let login = localStorage.getItem('login');
    let haslo = localStorage.getItem('haslo');
...
function dodaj_pilota() {
    ...
    $.post("/ajax/zamien_zaloge/", {'login': login, 'haslo': haslo, 'pilot_pk': pilot_pk, 'lot_pk': lot_pk})
        ...
        .fail(function (xhr, status, error) {
            alert("Przypisywanie pilota nie powiodło się!");
        });
